...
class Scrobblewindow(Window):
    ...
    def OnCombobox(self, event):
        if self.tmlist.GetSelection()==0:
            self.day.Show(True)
            self.day.SetValue(str(self.now.day))
            self.hour.Show(True)
    ...
    def _do_scrobble(self, event):
        if self.tmlist.GetSelection()==0:
            date= datetime.datetime(day=int(self.day.GetValue()), month=self.now.month, year=self.now.year, hour=int(self.hour.GetValue())+3, minute=int(self.minute.GetValue()),
second=20)
